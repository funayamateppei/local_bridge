...
本アプリケーションでは **Command パターン（操作ログ形式）** を採用しています。

### Command方式のメリット

1. **FEでの順序管理が不要**: 親エンティティの存在を気にせず操作を記録できる。同期時に種別順で実行するだけ
2. **タイムスタンプをローカルで発行**: `new Date().toISOString()` でオフライン中もUTC発行可能
3. **操作履歴がそのまま残る**: デバッグ時に何が起きたか追跡しやすい
4. **サーバー側もシンプル**: Commandを順番に適用するだけ
